...
The library is split into two main areas: application code and shader code.
...
The `rtxns::HostNetwork` object must be created and initialized before use. It can be initialized from input parameters describing the network architecture, from a file, or from another `rtxns::HostNetwork`. In each case, the network will be in a host layout (`rtxns::MatrixLayout::RowMajor` or `rtxns::MatrixLayout::ColumnMajor`).

```
// Initialise an empty network from parameters
nvrhi::IDevice* device = ...
auto networkUtils = std::make_shared<rtxns::NetworkUtilities>(device);
rtxns::HostNetwork hostNetwork(networkUtils);
...
The typical lifecycle of a network starts in a host layout, initialized with new values or loaded from a file. The network is then uploaded to the GPU and converted into a device-optimal layout using `rtxns::NetworkUtilities::ConvertWeights`. `rtxns::MatrixLayout::TrainingOptimal` is used while training; after training, another conversion can produce `rtxns::MatrixLayout::InferencingOptimal`. To store the trained network, convert it back to a host layout before writing it to a file so it can be shared between GPUs.
...
After creating a device-side buffer of `deviceNetworkLayout.networkByteSize`, use `rtxns::NetworkUtilities::ConvertWeights()` to convert the host layout to the device-optimal layout.
```
...
To integrate a custom neural network, start with `NetworkUtilities` in `NeuralNetwork.h`, which queries matrix sizes and converts device data between layouts and supported precisions. `GraphicsResources` in `GraphicsResources.h` reports the cooperative-vector features supported by the selected NVRHI device.
...
The linear operations module contains the main functions used for inference and training: `LinearOp` and `LinearOp_Backward`. It also provides an AutoDiff derivative for `LinearOp`.
...
The `LinearOp_Backward` function performs a backward linear step, applying a gradient of size `M` to the previous layer of size `K`. The weights, biases, and their derivatives are stored in their respective buffers. As with `LinearOp`, `CoopVecMatrixLayout` states the layout of the weight matrix, and `CoopVecComponentType` determines how the matrix should be interpreted.

```
CoopVec<T, K> LinearOp_Backward<
    T : __BuiltinFloatingPointType,
    let M : int,
    let K : int,
    let matrixLayout : CoopVecMatrixLayout,
...
The second half of this module extends cooperative vectors to support Slang AutoDiff.
...
`LinearOp_BackwardAutoDiff` is the backward derivative of `LinearOp`; its input `CoopVec` and `MatrixBiasBuffer` are passed in differential-pair types. See the Slang documentation for details.

```
void LinearOp_BackwardAutoDiff<
    T : __BuiltinFloatingPointType,
    let M : int,
    let K : int,
    let matrixLayout : CoopVecMatrixLayout,
...
The module contains an implementation of the Adam optimizer algorithm. `Adam` wraps a generic `AdamAccessor` and accepts two moment buffers plus the optimizer hyperparameters:
...
The encoder functions can be used to increase the input count of a neural network, providing additional information to assist learning. Validate their use to confirm that they improve quality or performance.

`CoopVecFromArray` constructs a matching-size `CoopVec` from a float array.
...
The `SanitizeEpsilon` function clamps a caller-provided `epsilon` into a safe, predefined range so it remains **positive** and **small**.
